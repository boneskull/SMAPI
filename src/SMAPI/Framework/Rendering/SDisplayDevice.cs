...
namespace StardewModdingAPI.Framework.Rendering
{
    ...
    internal class SDisplayDevice : SXnaDisplayDevice
    {
        ...
        *********/
        ...
        {
            return tile.Properties.TryGetValue("@Flip", out string? propertyValue) && int.TryParse(propertyValue, out int value)
                ? (SpriteEffects)value
                : SpriteEffects.None;
        }
        ...
        {
            if (!tile.Properties.TryGetValue("@Rotation", out string? propertyValue) || !int.TryParse(propertyValue, out int value))
                return 0;
